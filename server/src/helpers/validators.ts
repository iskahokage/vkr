...
const passMsg = 'Not valid password. Password must to contain at least: 1 number 1 symbol 1 uppercase letter'
export const registerValidation = [
    body("email").isEmail().withMessage("Not a valid e-mail address"),
    body("password").trim().isLength({ min: 4, max: 20 }).withMessage('Password must be 4-20 symbols').matches(/^(?=.*[A-Z])(?=.*[!@#$%^&*(),.?":{}|<>])(?=.*\d).+$/).withMessage(passMsg),
    ...
    body('legal_registered.region').notEmpty().withMessage("Please provide a region"),
    body('legal_registered.street').notEmpty().withMessage("Please provide a street"),
    body('legal_registered.house').notEmpty().withMessage("Please provide a house"),
    body('legal_registered.city').notEmpty().withMessage("Please provide a city"),
