...
const UserAddress: FC<UserAddressArgs> = ({ address, setUserData }) => {
    ...
    useEffect(() => {
        if (countries?.length === 0) dispatch(fetchCountries({ toast }));
    }, []);
    ...
    return (
        <>
            ...
            <div className="col-12 flex-wrap flex lg:col-3 lg:flex-nowrap">
                ...
                <Input value={address?.region} id="region" name="region" label="Регион" handleChange={handleChange} />
                <Input
                ...
                />
                <Input value={address?.city} id="city" name="city" label="Город" handleChange={handleChange} />
            </div>
            <div className="col-12 flex-wrap flex lg:col-3 lg:flex-nowrap">
                <Input
                    value={address?.resident_area}
                    id="resident_area"
                    name="resident_area"
                    label="Жилмассив"
                    handleChange={handleChange}
                />
                <Input value={address?.street} id="street" name="street" label="Улица" handleChange={handleChange} />
                <Input value={address?.house} id="house" name="house" label="Дом" handleChange={handleChange} />
                <Input value={address?.room} id="room" name="room" label="Квартира" handleChange={handleChange} />
            </div>
            <div className="col-12 flex-wrap flex lg:col-3 lg:flex-nowrap">
                <Input
